...
import type { IdempotencyStore } from "../src/idempotency/idempotencyStore.js";
...
vi.mock("../src/queue/qstashWebhook.js", () => ({
  QStashWebhookVerifier: class {
    ...
    }): Promise<{ ok: true; value: "verified" } | { ok: false; error: "invalid_signature" | "fatal" }> {
      return verifyMock(input) as Promise<
        { ok: true; value: "verified" } | { ok: false; error: "invalid_signature" | "fatal" }
      >;
    }
  }
}));
...
vi.mock("../src/worker/optimizerWorker.js", () => ({
  executeOptimizationPass: (payload: unknown) =>
    executeOptimizationPassMock(payload) as Promise<
      | { ok: true; value: { jobId: string; repository: string; rulesetVersion: string; findingsCount: number; gatesPassed: boolean } }
      | { ok: false; error: "retryable" | "fatal" }
    >
}));
...
describe("handleQStashRequest", () => {
  ...
  const createStore = (
    acquireResult: { ok: true; value: "acquired" } | { ok: false; error: "duplicate" | "fatal" }
  ): IdempotencyStore => ({
    acquire: vi.fn(async () => acquireResult),
    release: vi.fn(async () => ({ ok: true as const, value: "released" as const }))
  });
